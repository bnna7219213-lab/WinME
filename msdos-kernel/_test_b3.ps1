# _test_b3.ps1 — QEMU test for M-B3 (IOCP multiplexing)
$root = "C:\Users\bnna7\workspace\msdos-kernel"
$qemu = "C:\qemu\qemu-system-i386.exe"
$img  = Join-Path $root "winnt\winnt_b3.img"
$dbg  = Join-Path $root "winnt\b3_dbg.log"

# Rebuild
& "C:\mingw64\mingw64\bin\nasm.exe" -f bin -I $root `
    (Join-Path $root "winnt\src\b3_iocp.asm") `
    -o (Join-Path $root "winnt\build\b3-kernel.bin") | Out-Null
...
& "powershell.exe" -NoProfile -ExecutionPolicy Bypass -File (Join-Path $root "common\pack32.ps1") `
    ...
    -Kernel (Join-Path $root "winnt\build\b3-kernel.bin") `
